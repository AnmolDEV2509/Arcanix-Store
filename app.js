...
async function renderCheckoutPage() {
  ...
  window.setUpiTarget = (target) => {
    selectedApp = target;
    document.querySelectorAll('.upi-btn').forEach(btn => btn.classList.remove('active'));
    const activeBtn = document.getElementById(`upi-btn-${target}`);
    if (activeBtn) activeBtn.classList.add('active');
  };

  window.renderUpiAppSection = () => {
    ...
    payBox.innerHTML = `
      ...
      <div style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap; margin-bottom:10px;">
        <button type="button" id="upi-btn-gpay" onclick="setUpiTarget('gpay')" class="upi-btn ${selectedApp==='gpay'?'active':''}">Google Pay</button>
        <button type="button" id="upi-btn-phonepe" onclick="setUpiTarget('phonepe')" class="upi-btn ${selectedApp==='phonepe'?'active':''}">PhonePe</button>
        <button type="button" id="upi-btn-paytm" onclick="setUpiTarget('paytm')" class="upi-btn ${selectedApp==='paytm'?'active':''}">Paytm</button>
        <button type="button" id="upi-btn-generic" onclick="setUpiTarget('generic')" class="upi-btn ${selectedApp==='generic'?'active':''}">Any App</button>
      </div>
